...
(function ($) {
  ...
  function initTripSlide() {
    var $body = $('body');

    if ($body.hasClass('slickSliderTripActive')) return;
    $body.addClass('slickSliderTripActive');

    var $wrap = $('.b-tripadvisor');
    if (!$wrap.length) return;

    $wrap.find('.previews-wrap > div').slick({
      dots: false,
      infinite: true,
      arrows: false,
      speed: 500,
  ...
  //
  //  var $wrapper = $(".b-tripadvisor");
  //  if (!$wrapper.length) return;
  //
  //  var $body = $('body');
  //
  //  if ($body.hasClass('ScrollReviewsActive')) return;
  //  $body.addClass('ScrollReviewsActive');
  //
  ...
  function  initDot() {
    var $item = $('.b-tripadvisor .preview');

    if (!$item.length) return;

    var $body = $('body');

    if ($body.hasClass('ScrollDotActive')) return;
    $body.addClass('ScrollDotActive');
  ...
  function initElmsAnimation() {
    ...
    sr.reveal('.front .section-top .title-a .img', {
      ...
      scale: 0,
      rotate: { x: 0, y: 100, z: 0 },
      container: document.querySelector('.front .section-top .title-a')
    });
  ...
  function initTabsLocation() {
    ...
    var $nav = $wrapper.find('.nav > ul > li');
